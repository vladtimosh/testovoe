package org.example.manager; // Убедитесь, что это правильно

import org.example.dao.GenreDAO; // Импортируйте GenreDAO
import org.example.model.Genre; // Импортируйте Genre

import java.util.List;
import java.util.Scanner;

public class GenreManager {
    private final GenreDAO genreDAO;

    public GenreManager(GenreDAO genreDAO) {
        this.genreDAO = genreDAO;
    }

    public void manageGenres(Scanner scanner) {
        int genreChoice;

        do {
            displayMenu();
            genreChoice = getValidInteger(scanner);

            switch (genreChoice) {
                case 1 -> addGenre(scanner);
                case 2 -> viewGenre(scanner);
                case 3 -> viewAllGenres();
                case 4 -> updateGenre(scanner);
                case 5 -> deleteGenre(scanner);
                case 6 -> System.out.println("Возвращение в главное меню...");
                default -> System.out.println("Неверный выбор. Пожалуйста, попробуйте снова.");
            }
        } while (genreChoice != 6);
    }

    private void displayMenu() {
        System.out.println("\n--- Управление жанрами ---");
        System.out.println("1. Добавить жанр");
        System.out.println("2. Просмотреть жанр");
        System.out.println("3. Просмотреть все жанры");
        System.out.println("4. Обновить жанр");
        System.out.println("5. Удалить жанр");
        System.out.println("6. Вернуться в главное меню");
        System.out.print("Выберите опцию: ");
    }

    private void addGenre(Scanner scanner) {
        System.out.print("Введите название жанра: ");
        String genreName = scanner.nextLine().trim(); // Изменено на genreName

        Genre genre = new Genre(0, genreName); // Убедитесь, что Genre имеет правильный конструктор
        genreDAO.insertGenre(genre);
        System.out.println("Жанр успешно добавлен!");
    }

    private void viewGenre(Scanner scanner) {
        System.out.print("Введите ID жанра: ");
        int id = getValidInteger(scanner);
        Genre genre = genreDAO.getGenre(id);
        if (genre != null) {
            System.out.println(genre);
        } else {
            System.out.println("Жанр не найден.");
        }
    }

    private void viewAllGenres() {
        List<Genre> genres = genreDAO.getAllGenres();
        if (genres.isEmpty()) {
            System.out.println("Нет доступных жанров.");
        } else {
            genres.forEach(System.out::println);
        }
    }

    private void updateGenre(Scanner scanner) {
        System.out.print("Введите ID жанра для обновления: ");
        int id = getValidInteger(scanner);
        Genre genre = genreDAO.getGenre(id);
        if (genre != null) {
            System.out.print("Введите новое название жанра: ");
            String newGenreName = scanner.nextLine().trim(); // Изменено на newGenreName

            genre.setGenreName(newGenreName); // Изменено на setGenreName
            genreDAO.updateGenre(genre);
            System.out.println("Жанр успешно обновлен!");
        } else {
            System.out.println("Жанр не найден.");
        }
    }

    private void deleteGenre(Scanner scanner) {
        System.out.print("Введите ID жанра для удаления: ");
        int id = getValidInteger(scanner);
        genreDAO.deleteGenre(id);
        System.out.println("Жанр успешно удален!");
    }

    private int getValidInteger(Scanner scanner) {
        while (true) {
            try {
                return Integer.parseInt(scanner.nextLine().trim());
            } catch (NumberFormatException e) {
                System.out.print("Пожалуйста, введите целое число: ");
            }
        }
    }
}